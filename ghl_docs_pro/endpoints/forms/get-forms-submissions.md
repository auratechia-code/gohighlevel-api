...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/forms/submissions`
...
| `limit` | ❌ | formId string Filter submission by form id Example: jjusM6EOngDExnbo2DbU |
| `formId` | ❌ | q string Filter by contactId, name, email or phone no. Example: john@deo.com |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.forms.getFormsSubmissions({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/forms/submissions')
...
```python
	...
	conn.request("GET", "/forms/submissions", payload, headers)
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/forms/submissions', $headers);
...
```php
	...
	$request->setUrl('https://services.leadconnectorhq.com/forms/submissions');
...
```php
	...
	$request->setRequestUrl('https://services.leadconnectorhq.com/forms/submissions');
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/forms/submissions")
...
```ruby
	...
	url = URI("https://services.leadconnectorhq.com/forms/submissions")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/forms/submissions' -Method 'GET' -Headers $headers
