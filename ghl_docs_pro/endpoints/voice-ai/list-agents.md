...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/voice-ai/agents`
...
| :--- | :--- | :--- |
| `page` | ❌ | pageSize number Possible values: >= 1 and <= 50 Number of items per page Default value: 10 Example: 10 |
| `pageSize` | ✅ | locationId string required Location ID |
| `locationId` | ❌ | query string Query |
| `query` | ❌ |  |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.voiceAi.getAgents({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/voice-ai/agents')
...
```python
	...
	conn.request("GET", "/voice-ai/agents", payload, headers)
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/voice-ai/agents', $headers);
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/voice-ai/agents")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/voice-ai/agents' -Method 'GET' -Headers $headers
