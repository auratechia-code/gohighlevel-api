...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/invoices/schedule`
...
| `paymentMode` | ✅ | limit string required Limit the number of items to return Example: 10 |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.invoices.listInvoiceSchedules({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/invoices/schedule')
...
```python
	...
	conn.request("GET", "/invoices/schedule", payload, headers)
...
```php
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_URL => 'https://services.leadconnectorhq.com/invoices/schedule',
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/invoices/schedule', $headers);
...
```php
	...
	$request->setUrl('https://services.leadconnectorhq.com/invoices/schedule');
...
```php
	...
	$request->setRequestUrl('https://services.leadconnectorhq.com/invoices/schedule');
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/invoices/schedule")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/invoices/schedule' -Method 'GET' -Headers $headers
