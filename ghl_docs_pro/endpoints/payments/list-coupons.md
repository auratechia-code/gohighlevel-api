...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/payments/coupon/list`
...
| `altType` | ❌ | limit number Maximum number of coupons to return Default value: 100 Example: 10 |
| `limit` | ❌ | offset number Number of coupons to skip for pagination Default value: 0 Example: 0 |
| `offset` | ❌ | status string Possible values: [ scheduled , active , expired ] Filter coupons by status Example: active |
| `status` | ❌ | search string Search term to filter coupons by name or code Example: DEAL50 |
| `search` | ❌ |  |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.payments.listCoupons({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/payments/coupon/list')
...
```python
	...
	conn.request("GET", "/payments/coupon/list", payload, headers)
...
```php
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_URL => 'https://services.leadconnectorhq.com/payments/coupon/list',
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/payments/coupon/list', $headers);
...
```php
	...
	$request->setUrl('https://services.leadconnectorhq.com/payments/coupon/list');
...
```php
	...
	$request->setRequestUrl('https://services.leadconnectorhq.com/payments/coupon/list');
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/payments/coupon/list")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/payments/coupon/list' -Method 'GET' -Headers $headers
