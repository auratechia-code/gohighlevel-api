...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/payments/transactions`
...
| `altType` | ❌ | paymentMode string Mode of payment. Example: live |
| `paymentMode` | ❌ | startAt string Starting interval of transactions. Example: 2024-02-01 |
| `startAt` | ❌ | endAt string Closing interval of transactions. Example: 2024-02-13 |
| `endAt` | ❌ | entitySourceType string Source of the transactions. Example: funnel |
| `entitySourceType` | ❌ | entitySourceSubType string Source sub-type of the transactions. Example: two_step_order_form |
| `entitySourceSubType` | ❌ | search string The name of the transaction for searching. Example: Awesome transaction |
| `search` | ❌ | subscriptionId string Subscription id for filtering of transactions. Example: sub_1KGcXDCScnf89tZoVkoEMCEL |
| `subscriptionId` | ❌ | entityId string Entity id for filtering of transactions. Example: 61dd0fe9c077f73e67f78803 |
| `entityId` | ❌ | contactId string Contact id for filtering of transactions. Example: XPLSw2SVagl12LMDeTmQ |
| `contactId` | ❌ | limit number The maximum number of items to be included in a single page of results Default value: 10 Example: 20 |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.payments.listTransactions({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/payments/transactions')
...
```python
	...
	conn.request("GET", "/payments/transactions", payload, headers)
...
```php
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_URL => 'https://services.leadconnectorhq.com/payments/transactions',
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/payments/transactions', $headers);
...
```php
	...
	$request->setUrl('https://services.leadconnectorhq.com/payments/transactions');
...
```php
	...
	$request->setRequestUrl('https://services.leadconnectorhq.com/payments/transactions');
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/payments/transactions")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/payments/transactions' -Method 'GET' -Headers $headers
