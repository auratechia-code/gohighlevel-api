...
**Method:** `POST` | **URL:** `https://services.leadconnectorhq.com/store/shipping-zone`
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.store.createShippingZone({
...
```nodejs
	...
	var req = unirest('POST', 'https://services.leadconnectorhq.com/store/shipping-zone')
...
```python
	...
	conn.request("POST", "/store/shipping-zone", payload, headers)
...
```php
	...
	$request = new Request('POST', 'https://services.leadconnectorhq.com/store/shipping-zone', $headers, $body);
...
```java
	...
	HttpResponse<String> response = Unirest.post("https://services.leadconnectorhq.com/store/shipping-zone")
