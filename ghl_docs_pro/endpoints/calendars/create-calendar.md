...
**Method:** `POST` | **URL:** `https://services.leadconnectorhq.com/calendars/`
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.calendars.createCalendar({
...
```nodejs
	...
	var req = unirest('POST', 'https://services.leadconnectorhq.com/calendars/')
...
```python
	...
	conn.request("POST", "/calendars/", payload, headers)
...
```php
	...
	$request = new Request('POST', 'https://services.leadconnectorhq.com/calendars/', $headers, $body);
...
```java
	...
	HttpResponse<String> response = Unirest.post("https://services.leadconnectorhq.com/calendars/")
