...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/calendars/`
...
| :--- | :--- | :--- |
| `locationId` | ❌ | groupId string Group Id Example: BqTwX8QFwXzpegMve9EQ |
| `groupId` | ❌ | showDrafted boolean Show drafted Default value: true |
| `showDrafted` | ❌ |  |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.calendars.getCalendars({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/calendars/')
...
```python
	...
	conn.request("GET", "/calendars/", payload, headers)
...
```php
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_URL => 'https://services.leadconnectorhq.com/calendars/',
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/calendars/', $headers);
...
```php
	...
	$request->setUrl('https://services.leadconnectorhq.com/calendars/');
...
```php
	...
	$request->setRequestUrl('https://services.leadconnectorhq.com/calendars/');
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/calendars/")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/calendars/' -Method 'GET' -Headers $headers
