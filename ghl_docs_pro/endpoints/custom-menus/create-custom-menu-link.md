...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.customMenus.createCustomMenu({
...
```nodejs
	...
	var req = unirest('POST', 'https://services.leadconnectorhq.com/custom-menus/')
...
```python
	...
	conn.request("POST", "/custom-menus/", payload, headers)
...
```php
	...
	$request = new Request('POST', 'https://services.leadconnectorhq.com/custom-menus/', $headers, $body);
...
```java
	...
	HttpResponse<String> response = Unirest.post("https://services.leadconnectorhq.com/custom-menus/")
