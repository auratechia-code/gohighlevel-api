...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/users/`
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.users.getUserByLocation({

	    'locationId': 's4BtzHFWmT28mbb85uPa'
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/users/')
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/users/', $headers);
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/users/")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/users/' -Method 'GET' -Headers $headers
