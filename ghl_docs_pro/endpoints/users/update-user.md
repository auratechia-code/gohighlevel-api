...
**Method:** `PUT` | **URL:** `https://services.leadconnectorhq.com/users/:userId`
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.users.updateUser({
...
```nodejs
	...
	var req = unirest('PUT', 'https://services.leadconnectorhq.com/users/:userId')
...
```python
	...
	conn.request("PUT", "/users/:userId", payload, headers)
...
```php
	...
	$request = new Request('PUT', 'https://services.leadconnectorhq.com/users/:userId', $headers, $body);
...
```java
	...
	HttpResponse<String> response = Unirest.put("https://services.leadconnectorhq.com/users/:userId")
