...
**Method:** `GET` | **URL:** `https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId`
...
| Name | Required | Description |
| :--- | :--- | :--- |
| `pageSize` | ❌ | page number The page index for pagination. The default is 0. Default value: 0 Example: 0 |
| `page` | ❌ | searchFilter string Filter numbers by phone number pattern. Supports partial matching (e.g., "+91" to find all Indian numbers). Example: +91 |
| `searchFilter` | ❌ | skipNumberPool boolean Whether to exclude numbers that are assigned to number pools. Default is true. Default value: true Example: |
| `skipNumberPool` | ❌ |  |
...
```nodejs
	...
	try {
	  ...
	  const response = await highLevel.phoneSystem.activeNumbers({
...
```nodejs
	...
	var req = unirest('GET', 'https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId')
...
```python
	...
	conn.request("GET", "/phone-system/numbers/location/:locationId", payload, headers)
...
```php
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_URL => 'https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId',
...
```php
	...
	$request = new Request('GET', 'https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId', $headers);
...
```php
	...
	$request->setUrl('https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId');
...
```php
	...
	$request->setRequestUrl('https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId');
...
```java
	...
	HttpResponse<String> response = Unirest.get("https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId")
...
```powershell
	...
	$response = Invoke-RestMethod 'https://services.leadconnectorhq.com/phone-system/numbers/location/:locationId' -Method 'GET' -Headers $headers
